...
| profile-dashboard.wxss 合并去重（2159→989 行，0 !important） | ✅ |
...
| schedule-timeline.wxss 删除重复 alignment 层（1155→1070 行，0 !important） | ✅ |
| schedule.wxss 瘦身为纯骨架（498→3 行，空态并入 schedule-timeline） | ✅ |
| schedule 页扁平化唯一类名（sch-*，0 嵌套选择器） | ✅ |
| 全局 !important 清零（u-hide 替代） | ✅ |
| style:check 增加嵌套选择器检测 | ✅ |
| profile-dashboard / notes-memo / stats-ledger 嵌套扁平化 | 待续 |
| 核心路径人工验收清单 | 待本地确认 |

**Phase 5 指标（2026-07-08）：** 全局 `!important` **0**；`schedule-timeline.wxss` 日程区 **0 类嵌套**；`schedule.wxss` 使用 `time-duck` / `sch-*` 唯一类名。

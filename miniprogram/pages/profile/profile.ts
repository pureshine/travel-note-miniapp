...
Page({
  ...
  async loginWithWechat() {
    ...
    try {
      const profile = await loginByCloud();
  ...
  openProfileSetup(profile?: CloudProfile) {
    if (!profile) return;
  ...
  async restoreFromCloud() {
    ...
    try {
      const result = await downloadTripsFromCloud();
      this.applySyncResult(result.tripCount || 0, result.updatedAt);
      this.refreshLocalStats();
  ...
  applyProfile(profile?: CloudProfile) {
    if (!profile) {
      this.setData({
        loggedIn: false,
        nickname: "冲鸭旅行者",
  ...
  onChooseAvatar(event: { detail: { avatarUrl: string } }) {
    ...
    const profile = updateSavedProfile({ avatarUrl });
    if (profile) this.applyProfile(profile);
    syncTripsWithCloud().catch((error) => console.error("头像同步失败", error));
  },

  onSetupChooseAvatar(event: { detail: { avatarUrl: string } }) {
    this.setData({ draftAvatarUrl: event.detail.avatarUrl });
  },

  onSetupNicknameInput(event: { detail: { value: string } }) {
    this.setData({ draftNickname: event.detail.value });
  },

  onSetupNicknameBlur(event: { detail: { value: string } }) {
    this.setData({ draftNickname: event.detail.value });
  },
  ...
  saveProfileSetup() {
    ...
    const profile = updateSavedProfile({
      avatarUrl: this.data.draftAvatarUrl,
      nickname
    });
    if (profile) this.applyProfile(profile);
    this.setData({ showProfileSetup: false });
    syncTripsWithCloud().catch((error) => console.error("显示名字同步失败", error));
  ...
  refreshLocalStats() {
    ...
    const myOpenid = this.data.openid || getSavedProfile()?.openid || "";
  ...
  applyInvite(invite: InviteResult) {
    this.setData({
  ...
  applyAcceptedInvite(result: AcceptInviteResult) {
    this.setData({
